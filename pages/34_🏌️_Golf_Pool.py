...
def app():
    ...
    for col in ['Pick1', 'Pick2', 'Pick3', 'Pick4', 'Pick5', 'Pick6']:
        df_picks[col] = df_picks[col].str.replace('\d+', '',regex=True).str.replace(' `/','').str.replace('--','').str.replace(':','').str.replace(' /','').str.replace(' >','').str.strip().str.replace(' .%','').str.replace('Mcilroy','McIlroy').str.replace('Macintyre','MacIntyre').str.replace(' $,','').str.replace('$','').str.replace(',','').str.strip()

...
def get_s3_data():
    ...
    df = conn.read("bet-model-data/model_runs_streamlit.parquet", input_format="parquet", ttl=600)
    return df
...
def upload_s3_data(df, filename):
    ...
    s3.meta.client.upload_file(Filename=f'./{filename}.parquet', Bucket='bet-model-data', Key='model_runs_streamlit.parquet')
    st.write('data uploaded')


def visualize_model_over_time(df):
    ...
    fig=px.scatter(
        ...
        # color_discrete_sequence=['#FF6600','skyblue', 'red','yellow','blue'],
            ...
            hover_data=['matchup','ensemble_model_advice','time_to_tip','status','score_home','score_away','bet_status']
                )
    ...
    fig.update_traces(hovertemplate= "%{customdata[0]} -- tips in %{customdata[2]:.1f} mins<br>%{customdata[1]} -- Confidence: %{y:.1%}<br>%{customdata[3]} - %{customdata[4]} - %{customdata[5]} -- %{customdata[6]}" + "<extra></extra>")
...
def app():
    ...
    if input_pass==password:
        ...
        if time_since_last_model > 15:
            upload_s3_data(df, 'model_runs_streamlit')

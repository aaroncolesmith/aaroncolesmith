...
def visualize_model_over_time(df, num):
    ...
    matchups=df.sort_values(['time_to_tip','confidence_level'],ascending=[True,False]).head(num).matchup.unique()
...
def app():
    ...
    if input_pass==password:
        ...
        st.write(df1.loc[(df1.model_run==df1.model_run.max())&
                (df1.date==date.strftime('%Y-%m-%d'))&
                (df1.confidence_level>=.0002)][['id','status','start_time_et',
                                            'time_to_tip','matchup',
                                            'ensemble_model_advice','confidence_level','ensemble_model_win','score']].sort_values('time_to_tip'
                                            ,ascending=True).reset_index(drop=True).style.background_gradient(subset=['time_to_tip','confidence_level','ensemble_model_win']))


        matchups=df1.id.nunique()

        visualize_model_over_time(
            df1.loc[df1.date==date.strftime('%Y-%m-%d')].sort_values(['id','model_run'],ascending=[True,True]),
            matchups
            )

        id_list = df1.loc[df1.date==date.strftime('%Y-%m-%d')].id.unique()
        ...
        st.write(
            df1.loc[df1.id==id_list[0]][['status','model_run','time_to_tip','ensemble_model_advice','confidence_level','ttq']+four_feat_list].sort_values('model_run',ascending=True).style.background_gradient(axis=0)
        )

        with st.expander('All data for date:'):
           st.write(df1.loc[df1.date==date.strftime('%Y-%m-%d')].sort_values(['id','model_run'],ascending=[True,True]))
